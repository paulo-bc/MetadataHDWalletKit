...
public class UtxoSelector: UtxoSelectorInterface {
    ...
    public func select(from utxos: [UnspentTransaction], targetValue: UInt64) throws -> (utxos: [UnspentTransaction], fee: UInt64) {
        ...
        var numInputs = 2
        var fee: UInt64 {
            calculateFee(nIn: numInputs, nOut: numOutputs)
        }
        var targetWithFee: UInt64 {
            targetValue + fee
        }
        var targetWithFeeAndDust: UInt64 {
            targetWithFee + dustThreshhold
        }
...
extension Array {
    ...
    fileprivate func eachSlices(_ num: Int) -> [[Element]] {
...
extension Sequence where Element == UnspentTransaction {
    internal func sum() -> UInt64 {
        reduce(UInt64()) { $0 + $1.output.value }
    }
}
