...
public struct LegacyAddress: Address {
    ...
    public init(_ base58: Base58Check, coin: Coin) throws {
        ...
        self.type = type
        data = pubKeyHash.dropFirst()
        self.base58 = base58

        // cashaddr
        switch type {
        case .pubkeyHash:
            let payload = Data([coin.publicKeyHash]) + data
            cashaddr = Bech32.encode(payload, prefix: coin.scheme)
        case .wif:
            let payload = Data([coin.wifAddressPrefix]) + data
            cashaddr = Bech32.encode(payload, prefix: coin.scheme)
        default:
            cashaddr = ""
        }
