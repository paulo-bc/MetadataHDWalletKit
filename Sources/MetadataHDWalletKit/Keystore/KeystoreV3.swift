...
public class KeystoreV3: KeystoreInterface {
    ...
    ///
    public required convenience init?(data: Data, password: String) throws {
    ...
    public func encodedData() throws -> Data {
        try JSONEncoder().encode(keystoreParams)
    }
    ...
    public func getDecriptedKeyStore(passwordData: Data) throws -> Data? {
        ...
        guard let derivedKey = encryptData(
        ...
        guard let decryptedPK: [UInt8] = try? aesCipher.decrypt(cipherText.bytes) else { return nil }
    ...
    private func encryptDataToStorage(_ passwordData: Data, data: Data, dkLen: Int = 32, N: Int = 1024, R: Int = 8, P: Int = 1) throws {
        ...
        guard let derivedKey = encryptData(
            ...
            R: R,
            P: P
        ) else { throw KeystoreError.keyDerivationError }
        ...
        let kdfparams = KeystoreParamsV3.CryptoParamsV3.KdfParamsV3(
            salt: saltData.toHexString(), dklen: dkLen, n: N, p: P, r: R)
        let cipherparams = KeystoreParamsV3.CryptoParamsV3.CipherParamsV3(iv: IV.toHexString())
        let crypto = KeystoreParamsV3.CryptoParamsV3(
